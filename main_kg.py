...
if __name__ == '__main__':
    ...
    # Load datasets
    adj, features, train_dataloader, test_dataloader, _, num_train, num_test = graph_dataset.get_kg_datasets(opt)
    ...
    if opt.network == 'clip_classifier':
        video_model = transformer_network.CLIPClassifier(out_features=num_train, vit_backbone=opt.vit_backbone)
    ...
    if opt.network == 'c3d':
        ...
        nfeat_last_layer = 512
    model = graph_models.GAT(nfeat=features.shape[1], nclass=nfeat_last_layer, log_attention_weights=opt.log_attention_weights)
